...
const DrawerOverlay = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Overlay>>(({
  className,
  ...props
}, ref) => <DrawerPrimitive.Overlay ref={ref} className={cn("fixed inset-0 z-50 bg-black/60", className)} // Kept at 60% opacity
{...props} />);
...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({
  ...
  ...props
    ...
    <DrawerPrimitive.Content ref={ref} className={cn(
      // Changed to position at bottom and fixed width with margin
      "fixed bottom-0 left-0 right-0 z-50 mx-auto w-1/2 max-w-lg flex h-auto flex-col rounded-t-[10px] border bg-background",
      className
    )} {...props}>
