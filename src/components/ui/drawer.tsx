...
const DrawerOverlay = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Overlay>>(({
  className,
  ...props
}, ref) => <DrawerPrimitive.Overlay ref={ref} className={cn("fixed inset-0 z-50 bg-black/60", className)} // Changed opacity to 60%
{...props} />);
...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({
  ...
  ...props
    ...
    <DrawerPrimitive.Content ref={ref} className={cn("fixed left-1/4 right-1/4 bottom-1/4 z-50 w-1/2 mx-auto flex h-auto flex-col rounded-[10px] border bg-background", className)} {...props}>
