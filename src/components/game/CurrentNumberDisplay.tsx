...
export default function CurrentNumberDisplay({ 
  ...
  useEffect(() => {
    // Only update when we have a real number change, not just re-renders
    if (number !== null && number !== prevNumber) {
      logWithTimestamp(`CurrentNumberDisplay: New number ${number} (previous: ${prevNumber})`, 'debug');
      
      // Store the current number for animation/transition
    ...
    } else if (number !== null && prevNumber === null) {
      // Initial load with a number already available
      logWithTimestamp(`CurrentNumberDisplay: Initial number ${number}`, 'debug');
      setPrevNumber(number);
      setDisplayNumber(number);
    }
  }, [number, prevNumber]);
    ...
    <div className={`flex flex-col items-center justify-center ${className}`}>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={`flex items-center justify-center rounded-full shadow-lg ${getColor(displayNumber)} bingo-number-ball relative overflow-hidden`}
          style={{ 
            width: sizePx, 
            height: sizePx, 
            fontSize: sizePx * 0.4,
            boxShadow: displayNumber !== null ? '0 10px 25px -5px rgba(0, 0, 0, 0.2)' : 'none'
          }}
        >
