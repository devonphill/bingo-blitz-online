    ...
    try {
      ...
      if (existingProgress) {
        ...
        const { error: updateError } = await supabase
          .from('sessions_progress')
          .update({
            current_win_pattern: String(firstPattern.id),
            current_prize: String(firstPattern.prizeAmount),
            current_prize_description: String(firstPattern.description)
          })
          .eq('session_id', sessionId);
  ...
  const updatePatternPrizeInfo = useCallback(async (patternId: string) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('sessions_progress')
        .update({
          current_prize: String(pattern.prizeAmount || '10.00'),
          current_prize_description: String(pattern.description || `${patternId} Prize`)
        })
        .eq('session_id', sessionId);
